...
export function ReminderWidget({ initialReminders }: { initialReminders: Reminder[] }) {
  ...
  const [activeTab, setActiveTab] = useState<"reminders" | "wishes">("reminders");
  ...
  async function handleAddInline(e: React.FormEvent) {
    ...
    // Auto-prepend [Wish] if we are on the wishes tab
    const textToSave = activeTab === "wishes" ? `[Wish] ${newText.trim()}` : newText.trim();
  ...
  useEffect(() => {
    const channel = supabase
      .channel("dashboard_reminders_tabs")
      .on(
        "postgres_changes",
  ...
  const filteredCompleted = completedReminders.filter((r) => activeTab === "wishes" ? isWish(r.text) : !isWish(r.text));
    ...
    <div className="bg-white rounded-xl shadow-sm border border-border overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold text-sm text-gray-900 text-left">Tasks, Reminders & Wishes</h2>
          {totalCount > 0 && (
      ...
      {isExpanded && (
        <div className="p-4 pt-0 border-t border-border/60 space-y-4 bg-gray-50/10">
          ...
          {/* Segmented Tab Bar */}
          <div className="flex bg-gray-100/80 p-1 rounded-xl mt-4">
            <button
              onClick={() => { setActiveTab("reminders"); setShowCompleted(false); }}
            ...
            <button
              onClick={() => { setActiveTab("wishes"); setShowCompleted(false); }}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-bold rounded-lg transition-all ${
                activeTab === "wishes"
                  ? "bg-white text-gray-950 shadow-sm"
                  : "text-muted-foreground hover:text-gray-800"
            ...
            >
              ...
              <span>Wish List ({activeWishes.length})</span>
            </button>
          </div>
          ...
          <form onSubmit={handleAddInline} className="flex gap-2">
            <input
              ...
              placeholder={activeTab === "wishes" ? "Type a wishlist / pending work..." : "Type a fast site reminder..."}
            />
            <button
              type="submit"
              disabled={addingInline || !newText.trim()}
              className="h-10 px-3 bg-gray-950 text-white rounded-lg flex items-center justify-center disabled:opacity-40"
            >
          ...
          {/* RENDERING TABS */}
          {activeTab === "reminders" ? (
            /* --- standard REMINDERS VIEW --- */
            activeReminders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-6 text-center space-y-1">
                <p className="text-xs font-medium text-gray-950">No standard reminders! 🎉</p>
                <p className="text-[10px] text-muted-foreground">Type above or use + to add.</p>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                {groups.upcoming.length > 0 && (
                  <div className="space-y-1.5">
                    <p className="text-[9px] font-bold text-gray-500 tracking-wider uppercase">Upcoming</p>
            ...
            activeWishes.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center space-y-1">
                ...
                <p className="text-[10px] text-muted-foreground">Type a dream / backlog item above!</p>
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-[9px] font-bold text-amber-700 uppercase tracking-wider">Dream Scope & Pending Works</p>
                {activeWishes.map((w) => (
          ...
          <div className="pt-3 border-t border-border/40">
            ...
            >
              <span>{showCompleted ? "Hide Completed Items" : "Show Completed Items"}</span>
            ...
            {showCompleted && (
              <div className="mt-3 space-y-2">
                {isLoadingCompleted ? (
                  ...
                  <p className="text-[11px] text-muted-foreground italic text-center py-2">No completed items here yet.</p>
                ) : (
                      ...
                      <div className="flex items-start gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-xs text-gray-400 line-through break-words whitespace-normal leading-normal">
                            {activeTab === "wishes" ? cleanWishText(r.text) : r.text}
                          </p>
                        </div>
                      </div>
                      <button
